...
// Функция для отрисовки сырого текста ASCII на Canvas
  ...
  const handleUpload = async () => {
    ...
    setProcessing(true);
    setAsciiReady(false);
    setVideoResultUrl(null); // Сбрасываем старое видео перед новой загрузкой
    ...
    try {
      ...
      // Проверяем тип медиа, который вернул бэкенд
      if (data.media_type === "video") {
        setVideoResultUrl(data.result_url); // Сохраняем ссылку на видео
        setAsciiReady(true);
      } else {
        // Если это картинка, рисуем текст на Canvas как раньше
        const asciiText = data.ascii;
  ...
  const handleSaveToGallery = () => {
    ...
    const currentSrc = videoResultUrl || asciiDataUrl;
    if (!currentSrc) return;

    const isVideo = !!videoResultUrl;
    const ext = isVideo ? "mp4" : "png";
    const fileName = saveName.trim() || `ascii_${Date.now()}.${ext}`;
    
    // Для видео ставим примерный размер, для фото вычисляем из base64
    const estimatedSize = isVideo 
      ? 500 * 1024 
      : Math.max(10240, Math.ceil((asciiDataUrl.length - "data:image/png;base64,".length) * 3 / 4));
  ...
  const handleDownloadMedia = async () => {
    if (!asciiReady) return;
    const currentSrc = videoResultUrl || asciiDataUrl;
    const ext = videoResultUrl ? "mp4" : "png";
    if (!currentSrc) return;

    try {
      // Скачиваем файл как Blob, чтобы принудительно вызвать скачивание
      ...
      link.href = blobUrl;
      link.download = `ascii_${Date.now()}.${ext}`;
      ...
      <div className="flex flex-1 overflow-hidden" style={{ paddingTop: "36px" }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ padding: "16px 20px" }}>
          ...
          <div className="flex-1 relative border border-[#1e1e1e] overflow-hidden" style={{ background: "transparent", minHeight: "200px" }}>
            ...
            {/* Холст для статических изображений (скрываем, если это видео) */}
            <canvas
              ref={outputCanvasRef}
              className="absolute"
              style={{
                display: (asciiReady || processing) && !videoResultUrl ? "block" : "none",
            ...
            {/* Плеер для видео (показываем только если есть URL видео) */}
            {videoResultUrl && asciiReady && !processing && (
              <video 
                src={videoResultUrl} 
                autoPlay 
                loop 
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            {[
              { label: "КОПИЯ",   icon: <Copy size={11} />,        action: () => { 
                if (asciiReady) navigator.clipboard.writeText(videoResultUrl || asciiDataUrl); 
              } },
              { label: "ТЕКСТ",   icon: <Type size={11} /> },
              { label: "СКАЧАТЬ", icon: videoResultUrl ? <Film size={11} /> : <ImageIcon size={11} />, action: handleDownloadMedia },
